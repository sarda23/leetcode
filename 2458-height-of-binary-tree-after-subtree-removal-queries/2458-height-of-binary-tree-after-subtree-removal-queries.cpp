...
public:
    ...
    int findHt(TreeNode* root, int l){
        ...
        level[root->val] = l;
        height[root->val] = max(findHt(root->left,l+1),findHt(root->right,l+1)) + 1;
    ...
    vector<int> treeQueries(TreeNode* root, vector<int>& queries) {
        ...
        for(int &node : queries){
            ...
            int L = level[node]; // O(1) node ka level kya hai

            int temp_result = L + (levelMaxHt[L] == height[node] ? levelSecondMaxHt[L] : levelMaxHt[L]) - 1; 
